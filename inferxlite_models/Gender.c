/*
	...
	Softmax:
	inferx_softmax(int axis,char *bottom,char *top,char *name,char *model_pre, char *data_pre, struct inferx_handler *hd)
*/
...
void Gender(char* path, char* model, char* data, int *shape, int nshape, void* pdata, void** pout,int *len, struct inferx_handler *hd)
{
	inferx_net_preprocess(data,model,hd);

	inferx_input(shape,pdata,"data_data","data",model,data,hd);
	inferx_convolution(3,32,3,3,1,1,2,2,1,1,1,true,false,"data_data","conv1_1_scale_data","conv1_1",model,data, 1, 0.0, hd);
	inferx_pooling(2,2,2,2,0,0,"MAX","conv1_1_scale_data","pool1_data","pool1",model,data,hd);
	
	inferx_convolution(32,64,3,3,1,1,1,1,1,1,1,true,false,"pool1_data","conv2_1_scale_data","conv2_1",model,data, 1, 0.0, hd);
	inferx_pooling(2,2,2,2,0,0,"MAX","conv2_1_scale_data","pool2_data","pool2",model,data,hd);
	
	inferx_convolution(64,128,3,3,1,1,1,1,1,1,1,true,false,"pool2_data","conv3_1_scale_data","conv3_1",model,data, 1, 0.0, hd);
	inferx_pooling(2,2,2,2,0,0,"MAX","conv3_1_scale_data","pool3_data","pool3",model,data,hd);
	
	inferx_convolution(128,128,3,3,1,1,1,1,1,1,1,true,false,"pool3_data","conv4_1_scale_data","conv4_1",model,data, 1, 0.0, hd);
	inferx_pooling(2,2,2,2,0,0,"MAX","conv4_1_scale_data","pool4_data","pool4",model,data,hd);
	
	inferx_inner_product(128,128,true,false,"pool4_data","ip1_data","ip1_finetune",model,data,hd);
	inferx_relu(0.0,"ip1_data","ip1_data","relu_ip1",model,data,hd);
	inferx_inner_product(128,2,true,false,"ip1_data","ip3_data","ip3_finetune",model,data,hd);
	inferx_softmax(1,"ip3_data","gender_prob_data","gender_prob",model,data,hd);

	// only one output feature map 
	//inferx_sort_data("gender_prob_data",data);
	//inferx_print_data("gender_prob_data",data);
	//if(pout)
	  //*pout = inferx_get_data("gender_prob_data",data,len,hd);
	inferx_finalize("Gender",hd);
