...
static inline float32x4_t simd_tanh4f(const float32x4_t a)
{
    float32x4_t x, y, z, b;
    float32x4_t tem0, tem1, tem2, tem3;
    const float32x4_t one = *(float32x4_t *) _ps128_1;
    const float32x4_t zero = simd_zeros;
    const float32x4_t mask = *(float32x4_t *) _ps128_sign_mask;
    float32x4_t sign_bit;
     ...
     * }
     */
    tem1 = simd_cmplts(x, *(float32x4_t *) _ps128_tanh_0p625);
    tem2 = simd_adds(x, x);

    // call exponential function
    z = simd_exp4f(tem2);

    tem2 = simd_subs(one, simd_divs(*(float32x4_t *) _ps128_2, simd_adds(one, z)));
	...
	tem0 = simd_muls(x, x);
    y = simd_mas(*(float32x4_t *) _ps128_tanh_p0, tem0, *(float32x4_t *) _ps128_tanh_p1);
    tem3 = simd_mas(y, tem0, *(float32x4_t *) _ps128_tanh_p2);
    y = simd_mas(tem3, tem0, *(float32x4_t *) _ps128_tanh_p3);
    tem3 = simd_mas(y, tem0, *(float32x4_t *) _ps128_tanh_p4);
    ...
    tem0 = simd_cmples(x, *(float32x4_t *) _ps128_tanh_half_max);
